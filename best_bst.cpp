...
class map_bst {
  ...
  public:
    ...
    bool checkParent() { return checkParent(mRoot); }
    ...
    bool checkInorder() { return checkInorder(mRoot); }
...
void gen_best_bst_recuir(std::vector<int> &data, int start, int end, int mid) {
    data.push_back(mid);
    ...
    gen_best_bst_recuir(data, start, mid - 1, (start + mid - 1) / 2);
    gen_best_bst_recuir(data, mid + 1, end, (mid + 1 + end) / 2);
}

void gen_best_bst(int n, CP::map_bst<int, int> &bst) {
    std::vector<int> data;

    gen_best_bst_recuir(data, 1, n, (n + 1) / 2);

    for (int i = 1; i <= n; i++) {
        bst[data[i - 1]] = 100; // ค่า value up to you
    }
}
...
int main() {
    int k;
    int n;
    std::cin >> k; // สร้างต้นไม้ความลึก = k
    n = (2 << k) - 1; // คำนวณหาจำนวน node จากความลึก k
